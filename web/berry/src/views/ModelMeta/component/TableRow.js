...
import { renderNumber, timestamp2string } from 'utils/common';
  ...
  return (
    <>
      <TableRow tabIndex={item.id}>
        ...
        <TableCell>{renderRatio(item.channel_type, item.model_ratio)}</TableCell>
        <TableCell>{renderRatio(item.channel_type, item.completion_ratio)}</TableCell>
...
function renderRatio(channel_type, balance) {
  switch (channel_type) {
    case 1: // OpenAI
    ...
    case 8: // 自定义
      return <span>${balance.toFixed(5)}</span>;
    case 5: // OpenAI-SB
      return <span>¥{(balance / 10000).toFixed(5)}</span>;
    case 10: // AI Proxy
