...
const LogsTable = () => {
  ...
  const loadLogs = async (startIdx, pageSize, logType = 0) => {
    ...
    let localEndTimestamp = Date.parse(end_timestamp) / 1000;
    if (isAdminUser) {
      url = `/api/log/?p=${startIdx}&page_size=${pageSize}&type=${logType}&username=${username}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&channel=${channel}&session_id=${session_id}`;
    } else {
      url = `/api/log/self/?p=${startIdx}&page_size=${pageSize}&type=${logType}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&session_id=${session_id}`;
    }
    ...
    <Layout>
      ...
      <Form layout="horizontal" style={{ marginTop: 10 }}>
        <>
          ...
          <Form.Input field="session_id" label="会话ID" style={{ width: 176 }} value={session_id}
            placeholder="可选值"
            name="session_id"
            onChange={value => handleInputChange(value, 'session_id')} />
