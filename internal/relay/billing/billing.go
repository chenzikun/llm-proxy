...
func PostConsumeQuota(ctx context.Context, tokenId int, quotaDelta int64, totalQuota int64, userId int, channelId int, modelRatio float64, groupRatio float64, modelName string, tokenName string, sessionId string) {
	...
	if totalQuota != 0 {
		...
		model.RecordConsumeLog(ctx, userId, channelId, int(totalQuota), 0, 0, modelName, tokenName, totalQuota, 0, logContent, sessionId)
