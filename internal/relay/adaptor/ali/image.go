...
func ImageHandler(c *gin.Context, resp *http.Response) (*objects.ErrorWithStatusCode, *entity.Usage) {
	...
	if aliTaskResponse.Message != "" {
		...
		return objects.ErrorWrapper(errors.New(aliTaskResponse.Message), "ali_async_task_failed", http.StatusInternalServerError), nil
	}

	aliResponse, _, err := asyncTaskWait(aliTaskResponse.Output.TaskId, apiKey)
	if err != nil {
		return objects.ErrorWrapper(err, "ali_async_task_wait_failed", http.StatusInternalServerError), nil
	}
