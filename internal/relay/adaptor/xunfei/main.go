...
func requestOpenAI2Xunfei(request entity.GeneralOpenAIRequest, xunfeiAppId string, domain string) *ChatRequest {
	...
	if strings.HasPrefix(domain, "generalv3") || domain == "4.0Ultra" {
		functions := make([]entity.Function, len(request.Tools))
...
func getToolCalls(response *ChatResponse) []entity.Tool {
	var toolCalls []entity.Tool
...
func StreamHandler(c *gin.Context, meta *objects.Meta, textRequest entity.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*objects.ErrorWithStatusCode, *entity.Usage) {
	domain, authUrl := getXunfeiAuthUrl(meta.Config.APIVersion, apiKey, apiSecret)
	dataChan, stopChan, err := xunfeiMakeRequest(textRequest, domain, authUrl, appId)
	if err != nil {
		return objects.ErrorWrapper(err, "xunfei_request_failed", http.StatusInternalServerError), nil
	}
...
func Handler(c *gin.Context, meta *objects.Meta, textRequest entity.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*objects.ErrorWithStatusCode, *entity.Usage) {
	domain, authUrl := getXunfeiAuthUrl(meta.Config.APIVersion, apiKey, apiSecret)
	dataChan, stopChan, err := xunfeiMakeRequest(textRequest, domain, authUrl, appId)
	if err != nil {
		return objects.ErrorWrapper(err, "xunfei_request_failed", http.StatusInternalServerError), nil
	...
	if len(xunfeiResponse.Payload.Choices.Text) == 0 {
		return objects.ErrorWrapper(errors.New("xunfei empty response detected"), "xunfei_empty_response_detected", http.StatusInternalServerError), nil
	}
...
func xunfeiMakeRequest(textRequest entity.GeneralOpenAIRequest, domain, authUrl, appId string) (chan ChatResponse, chan bool, error) {
	d := websocket.Dialer{
