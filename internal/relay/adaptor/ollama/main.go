...
// getOllamaModelName 将代理模型名转换为真实 Ollama 模型名。
// 若 ModelMap 中无对应映射，则直接将代理名透传给 Ollama（兼容直接使用真实模型名的场景）。
// 也可通过 Channel.ModelMapping 将代理名预先映射为真实名再传入此处。
func getOllamaModelName(proxyModelName string) string {
	if realName, ok := ModelMap[proxyModelName]; ok {
		return realName
	}
	return proxyModelName
}

func ConvertRequest(request entity.GeneralOpenAIRequest) *ChatRequest {
	ollamaRequest := ChatRequest{
		Model: getOllamaModelName(request.Model),
		Options: &Options{
...
func ConvertEmbeddingRequest(request entity.GeneralOpenAIRequest) *EmbeddingRequest {
...
func EmbeddingHandler(c *gin.Context, resp *http.Response) (*objects.ErrorWithStatusCode, *entity.Usage) {
...
func embeddingResponseOllama2OpenAI(response *EmbeddingResponse) *openai.EmbeddingResponse {
	openAIEmbeddingResponse := openai.EmbeddingResponse{
		...
		Model:  response.Model,
		Usage:  entity.Usage{TotalTokens: 0},
	}
...
func Handler(c *gin.Context, resp *http.Response) (*objects.ErrorWithStatusCode, *entity.Usage) {
	ctx := context.TODO()
	var ollamaResponse ChatResponse
