...
func SetWebRouter(router *gin.Engine, buildFS embed.FS) {
	finetuneIndexPage, err := buildFS.ReadFile("web/doc/api/finetune/index.html")
	if err != nil {
	...
	// Serve static files
	docFS := common.EmbedFolder(buildFS, "web/doc")
	docServer := static.Serve("/doc", docFS)
	staticServer := static.Serve("/", common.EmbedFolder(buildFS, fmt.Sprintf("web/build/%s", config.Theme)))
