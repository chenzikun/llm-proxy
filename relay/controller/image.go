...
import (
	...
	relaymodel "github.com/songquanpeng/one-api/relay/entity"
)
...
func validateImageRequest(imageRequest *relaymodel.ImageRequest, meta *objects.Meta) *objects.ErrorWithStatusCode {
	// check prompt length
	if imageRequest.Prompt == "" {
		return objects.ErrorWrapper(errors.New("prompt is required"), "prompt_missing", http.StatusBadRequest)
	}

	// model validation
	if !isValidImageSize(imageRequest.Model, imageRequest.Size) {
		return objects.ErrorWrapper(errors.New("size not supported for this image model"), "size_not_supported", http.StatusBadRequest)
	}

	if !isValidImagePromptLength(imageRequest.Model, len(imageRequest.Prompt)) {
		return objects.ErrorWrapper(errors.New("prompt is too long"), "prompt_too_long", http.StatusBadRequest)
	}

	// Number of generated images validation
	if !isWithinRange(imageRequest.Model, imageRequest.N) {
		return objects.ErrorWrapper(errors.New("invalid value of n"), "n_not_within_range", http.StatusBadRequest)
	}
	return nil
}
...
func RelayImageHelper(c *gin.Context, relayMode int) *objects.ErrorWithStatusCode {
	ctx := c.Request.Context()
	meta := objects.GetRequestMeta(c)
	imageRequest, err := getImageRequest(c, meta.Mode)
	if err != nil {
		logger.Errorf(ctx, "getImageRequest failed: %s", err.Error())
		return objects.ErrorWrapper(err, "invalid_image_request", http.StatusBadRequest)
	}
	...
	if isModelMapped || meta.ChannelType == channeltype.Azure { // make Azure channel request body
		...
		if err != nil {
			return objects.ErrorWrapper(err, "marshal_image_request_failed", http.StatusInternalServerError)
		}
	...
	if err := adaptor.Init(meta); err != nil {
		return objects.ErrorWrapper(err, "init adaptor failed", http.StatusInternalServerError)
	}
	...
	case channeltype.Zhipu:
		...
		if err != nil {
			return objects.ErrorWrapper(err, "convert_image_request_failed", http.StatusInternalServerError)
		}
	...
	defer func(ctx context.Context) {
		...
		if quota != 0 {
			...
			model.RecordConsumeLog(ctx, meta.UserId, meta.ChannelId, 0, 0, imageRequest.Model, tokenName, quota, logContent, meta.SessionId)
